...
2. 必须调用 preview_ingestion_chunking 生成并比较候选；最多可保存 3 个不同候选，重复配置会复用结果。
3. 可并行预览候选。观察真实 diagnostics、块长度、结构保持与五维评分后再修正。
4. 最终必须调用 submit_ingestion_decision，并且 candidate_id 必须来自成功预览且通过硬校验的候选。
