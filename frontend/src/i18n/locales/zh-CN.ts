export default {
  ...
  integrations: {
    ...
    api: {
      ...
      personalTitle: '个人 API 集成',
      personalSubtitle: '使用你自己的密钥检索已授权知识库并发起对话；不会获得模型、成员或空间管理权限。',
      myApiKeys: '我的 API Keys',
      personalKeysHint: '个人密钥固定具备检索与对话能力，且只能访问创建时明确选择的知识库。',
      personalFixedCapabilities: '检索 + 对话',
      personalOwner: '所属成员',
      personalExamples: '调用示例',
      personalExamplesHint: '示例使用你的第一个密钥和已授权知识库，可复制后替换问题内容。',
      personalHistory: '个人 API 会话历史',
      personalHistoryHint: '这里只显示个人 API Key 产生的会话，不包含网页聊天和其他成员的会话。',
      personalHistorySearch: '搜索会话标题',
      personalHistoryEmpty: '暂无个人 API 会话',
      personalHistorySelect: '选择一个会话查看消息',
      personalHistoryLoadFailed: '加载个人 API 会话失败',
      personalHistoryLoadMore: '加载更多消息',
      personalMessagesEmpty: '该会话暂无消息',
      personalUntitledSession: '未命名会话',
      personalRoleUser: '用户',
      personalRoleAssistant: '助手',
      personalKnowledgeRequired: '至少选择一个可读取的知识库',
      personalKnowledgeLoadFailed: '加载可用知识库失败',
      personalKnowledgeHint: '密钥不会自动获得未来新增知识库的权限；需要时请重新创建或单独授权。',
      personalCreateRequired: '请填写名称并至少选择一个知识库',
      personalTokenCreated: '个人 API Key 已创建',
      personalTokenCreatedHint: '请立即复制并妥善保存该密钥。',
      apiKeys: 'API Keys',
  ...
  datasource: {
    ...
    hoursAgo: '{n} 小时前',
    daysAgo: '{n} 天前',
    createSuccessManualSync_wecom_wedrive: '企业微信微盘数据源已创建；文件同步将在后续同步管线启用后手动触发',
    create: '创建',
    resourceType: {
      wikiSpace: '知识库空间',
      docCategory: '文档标签',
      book: '语雀知识库',
      wecomWeDriveSpace: '微盘空间',
      wecomWeDriveFolder: '文件夹',
      wecomWeDriveFile: '文件'
    },
    scheduleHuman: {
      '30min': '每 30 分钟',
    ...
    field: {
      ...
      wecomSpaceIds: '微盘 Space ID',
      wecomSpaceIdsHint: '每行一个微盘空间 ID。第一版通过已知 space id 浏览空间，适合按部门公共空间创建独立知识库。',
      corpId: '企业 ID',
      wecomSecret: '微盘 Secret',
      wecomUserId: '操作者 UserID',
      wecomUserIdHint: '用于调用微盘 API 的企业微信成员 userid，需要对目标微盘空间有读取权限'
    },
    connectorDesc: {
  ...
  model: {
    ...
    editor: {
      ...
      providers: {
        ...
        ollama_cloud: {
          label: 'Ollama Cloud',
          description: '通过 OpenAI 兼容接口使用 Ollama 云端托管模型'
        }
      },
      validation: {
  ...
  knowledgeStages: {
    ...
    errorCode: {
      ...
      INGESTION_MODEL_TOOL_CALLING_UNSUPPORTED_SUGGESTION: '改用支持原生 tool calling 的摘要模型后重试，或使用知识库切分配置。',
      INGESTION_CORE_TOOL_FAILED: '入库核心工具失败',
      INGESTION_CORE_TOOL_FAILED_SUGGESTION: '查看原始错误和应用日志，修复工具依赖后智能重试。',
      INGESTION_CANDIDATE_INVALID: '切分候选无效',
      INGESTION_CANDIDATE_INVALID_SUGGESTION: 'Agent 提交了未预览或未通过硬校验的候选；可智能重试或改用知识库配置。',
      INGESTION_TOOL_FAILED: '入库工具执行失败',
      INGESTION_TOOL_FAILED_SUGGESTION: '查看错误类别和应用日志，修复工具故障后重试。',
      INGESTION_TOOL_ARGUMENTS_INVALID: '入库工具参数不符合约束',
      INGESTION_TOOL_ARGUMENTS_INVALID_SUGGESTION: '查看错误中的字段和约束，改用兼容的工具调用模型后重试。',
      INGESTION_CANDIDATE_PREVIEW_FAILED: '切分预览硬校验失败',
      INGESTION_CANDIDATE_PREVIEW_FAILED_SUGGESTION: '生产切分器的位置、顺序或父子映射校验失败；请查看错误约束和应用日志。',
      INGESTION_CANDIDATE_LIMIT_REACHED: '切分候选已达到上限',
      INGESTION_CANDIDATE_LIMIT_REACHED_SUGGESTION: 'Agent 达到候选上限后仍继续预览；请重试并提交已有有效候选。',
      INGESTION_DECISION_INVALID: '入库切分决策无效',
      INGESTION_DECISION_INVALID_SUGGESTION: '提交的候选未成功预览或未通过校验；可智能重试或改用知识库配置。',
      ...
      INGESTION_AGENT_EXECUTION_FAILED_SUGGESTION: '查看原始错误和应用日志，修复模型或工具服务后重试。',
    ...
    analysis: {
      ...
      recommended: '推荐',
      applied: '实际应用',
      agentRunTitle: 'Agent 运行轨迹',
      rounds: '第 {actual} / {max} 轮',
      candidatesTitle: '候选评分比较',
  ...
  uploadConfirm: {
    ...
    navChunkingSmart: '由 Agent 决定',
    processingModeLabel: '切分处理方式',
    processingModeDescription: '智能分析会逐文件判断文档画像并生成切分参数。',
    modeSmart: '智能分析',
    modeKnowledgeBase: '知识库配置',
    smartDecisionHint: '切分策略、块大小、重叠、父子块和分隔符将由 Agent 决定。',
    smartModelMissing: '当前知识库未配置摘要模型，智能分析会明确失败。请先配置模型或改用知识库配置。',
    smartUrlsUseKb: '本批次中的 URL 不启用智能分析，仍按知识库配置处理。',
    smartWebAccess: '允许使用网络搜索与抓取',
    smartWebAccessDescription: '开启后，Agent 可将查询或所选原文片段发送给已配置的外部 Web 服务。',
    smartMcpAccess: '允许使用只读 MCP 工具',
    smartMcpAccessDescription: '开启后，Agent 可将查询或原文片段发送给明确声明 readOnlyHint=true 的 MCP 工具；后台不会等待交互授权。',
    agentOwned: '由 Agent 决定',
    statusOn: '已开启',
    statusOff: '未开启',
    notSet: '未设置',
    summaryNoTags: '未设置',
