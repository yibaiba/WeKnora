export default {
  ...
  integrations: {
    ...
    api: {
      ...
      personalTitle: 'Персональная API-интеграция',
      personalSubtitle: 'Используйте личный ключ для поиска в разрешённых базах знаний и чата без прав управления моделями, участниками или пространством.',
      myApiKeys: 'Мои API-ключи',
      personalKeysHint: 'Личные ключи имеют только поиск и чат и работают лишь с базами знаний, выбранными при создании.',
      personalFixedCapabilities: 'Поиск + чат',
      personalOwner: 'Участник',
      personalExamples: 'Примеры запросов',
      personalExamplesHint: 'Примеры используют первый ключ и разрешённую базу знаний.',
      personalHistory: 'Личная история API',
      personalHistoryHint: 'Здесь только сессии личных API-ключей; веб-чаты и сессии других участников исключены.',
      personalHistorySearch: 'Поиск по заголовку',
      personalHistoryEmpty: 'Личных API-сессий пока нет',
      personalHistorySelect: 'Выберите сессию для просмотра сообщений',
      personalHistoryLoadFailed: 'Не удалось загрузить личные API-сессии',
      personalHistoryLoadMore: 'Загрузить ещё сообщения',
      personalMessagesEmpty: 'В этой сессии нет сообщений',
      personalUntitledSession: 'Сессия без названия',
      personalRoleUser: 'Пользователь',
      personalRoleAssistant: 'Ассистент',
      personalKnowledgeRequired: 'Выберите хотя бы одну доступную базу знаний',
      personalKnowledgeLoadFailed: 'Не удалось загрузить доступные базы знаний',
      personalKnowledgeHint: 'Ключ не получает автоматически доступ к будущим базам знаний.',
      personalCreateRequired: 'Введите название и выберите хотя бы одну базу знаний',
      personalTokenCreated: 'Личный API-ключ создан',
      personalTokenCreatedHint: 'Скопируйте и безопасно сохраните ключ сейчас.',
      apiKeys: 'API Keys',
  ...
  datasource: {
    ...
    hoursAgo: '{n} ч назад',
    daysAgo: '{n} д назад',
    createSuccessManualSync_wecom_wedrive: 'Enterprise WeChat Microdisk data source created. File sync can be triggered manually after the sync pipeline is enabled.',
    create: 'Create',
    resourceType: {
      wikiSpace: 'Пространство вики',
      docCategory: 'Тег документа',
      book: 'База знаний Yuque',
      wecomWeDriveSpace: 'Microdisk Space',
      wecomWeDriveFolder: 'Folder',
      wecomWeDriveFile: 'File'
    },
    scheduleHuman: {
      '30min': 'Каждые 30 мин',
      '1h': 'Каждый час',
    ...
    field: {
      ...
      wecomSpaceIds: 'Microdisk Space IDs',
      wecomSpaceIdsHint: 'One Microdisk space ID per line. The first version browses known space IDs, suitable for one department-public space per knowledge base.',
      corpId: 'Corp ID',
      wecomSecret: 'Microdisk Secret',
      wecomUserId: 'Operator UserID',
      wecomUserIdHint: 'Enterprise WeChat userid used for Microdisk API calls. This user must be able to read the target space.'
    },
    connectorDesc: {
  ...
  model: {
    ...
    editor: {
      ...
      contextWindowTokensDetected: 'Автоматически установлено {value} токенов по поставщику и имени модели; при необходимости укажите лимит развёртывания вручную.',
      ...
      providers: {
        ...
        ollama_cloud: {
          label: 'Ollama Cloud',
          description: 'Облачные модели Ollama через OpenAI-совместимый API'
        }
      },
      validation: {
  ...
  knowledgeStages: {
    ...
    errorCode: {
      ...
      INGESTION_MODEL_TOOL_CALLING_UNSUPPORTED_SUGGESTION: 'Выберите модель с нативной поддержкой tool calling и повторите анализ либо используйте настройки базы.',
      INGESTION_CORE_TOOL_FAILED: 'Ошибка основного инструмента загрузки',
      INGESTION_CORE_TOOL_FAILED_SUGGESTION: 'Проверьте исходную ошибку и логи, исправьте зависимость инструмента и выполните умный повтор.',
      ...
      INGESTION_TOOL_FAILED: 'Ошибка инструмента загрузки',
      INGESTION_TOOL_FAILED_SUGGESTION: 'Проверьте категорию ошибки и логи приложения, устраните сбой инструмента и повторите анализ.',
      INGESTION_TOOL_ARGUMENTS_INVALID: 'Недопустимые параметры инструмента загрузки',
      INGESTION_TOOL_ARGUMENTS_INVALID_SUGGESTION: 'Проверьте указанное поле и ограничение, затем повторите анализ с совместимой моделью tool calling.',
      INGESTION_CANDIDATE_PREVIEW_FAILED: 'Ошибка строгой проверки предварительного разбиения',
      INGESTION_CANDIDATE_PREVIEW_FAILED_SUGGESTION: 'Нарушен инвариант позиции, порядка или связи родительских и дочерних блоков. Проверьте ограничение и логи.',
      INGESTION_CANDIDATE_LIMIT_REACHED: 'Достигнут лимит кандидатов разбиения',
      INGESTION_CANDIDATE_LIMIT_REACHED_SUGGESTION: 'Agent продолжил предварительный просмотр после достижения лимита. Повторите анализ для отправки существующего кандидата.',
      INGESTION_DECISION_INVALID: 'Недопустимое решение о загрузке',
      INGESTION_DECISION_INVALID_SUGGESTION: 'Отправленный кандидат не был успешно просмотрен или проверен. Повторите анализ либо используйте настройки базы.',
      ...
      INGESTION_AGENT_EXECUTION_FAILED_SUGGESTION: 'Проверьте исходную ошибку и логи, исправьте модель или сервис инструмента и повторите анализ.',
    ...
    analysis: {
      ...
      applied: 'Применено',
      agentRunTitle: 'Трассировка выполнения Agent',
      rounds: 'Раунд {actual} / {max}',
      candidatesTitle: 'Сравнение оценок кандидатов',
      ...
      tool: {
        ...
        list_knowledge_chunks: 'Чтение чанков знаний',
        query_knowledge_graph: 'Запрос к графу знаний',
        get_document_info: 'Чтение сведений о документе',
        database_query: 'Запрос к БД только для чтения',
        data_analysis: 'Анализ данных',
        data_schema: 'Чтение схемы данных',
        web_search: 'Поиск в интернете',
        web_fetch: 'Загрузка веб-страницы',
        wiki_read_page: 'Чтение страницы Wiki',
        wiki_search: 'Поиск в Wiki',
        wiki_read_source_doc: 'Чтение источника Wiki',
        wiki_read_issue: 'Чтение issue Wiki'
      },
      documentKind: {
  ...
  uploadConfirm: {
    ...
    navChunkingSmart: 'Определяет Agent',
    processingModeLabel: 'Режим разбиения',
    processingModeDescription: 'Интеллектуальный анализ определяет профиль каждого файла и выбирает параметры разбиения.',
    modeSmart: 'Интеллектуальный анализ',
    modeKnowledgeBase: 'Настройки базы знаний',
    smartDecisionHint: 'Agent выберет стратегию, размеры, перекрытие, родительско-дочерний режим и разделители.',
    smartModelMissing: 'Для базы знаний не настроена модель суммаризации. Интеллектуальный анализ завершится явной ошибкой; настройте модель или используйте настройки базы.',
    smartUrlsUseKb: 'URL в этой партии не используют интеллектуальный анализ и обрабатываются с настройками базы знаний.',
    smartWebAccess: 'Разрешить веб-поиск и загрузку страниц',
    smartWebAccessDescription: 'После включения Agent может отправлять запросы или выбранные фрагменты исходного текста настроенным внешним Web-сервисам.',
    smartMcpAccess: 'Разрешить MCP-инструменты только для чтения',
    smartMcpAccessDescription: 'После включения Agent может отправлять запросы или фрагменты текста MCP-инструментам с readOnlyHint=true; фоновая обработка не ждёт интерактивной авторизации.',
    agentOwned: 'Определяет Agent',
    statusOn: 'Вкл.',
    statusOff: 'Выкл.',
    notSet: 'Не задано',
